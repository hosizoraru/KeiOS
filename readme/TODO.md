...
- [ ] Run API 36 16 KB page-size validation on a packaged debug or benchmark APK, including transitive native libraries from dependencies; record any page-size compat warning, offending `.so`, and temporary `android:pageSizeCompat` decision.
- [ ] Run API 36 ART / native-dependency smoke after fresh install and upgrade: app startup, MMKV read/write, cache clear, Coil GIF decode, Media3 BGM/video, Shizuku init, focus-api notification build, and backdrop/liquid-glass rendering; capture `UnsatisfiedLinkError`, ART internal-structure, and hidden-API stack traces.
...
- [ ] Validate API 36 SAF / URI-grant flows for non-Home background `OpenDocument -> uCrop`, BA media custom save location, BA ZIP export, and log archive export; keep the permissionless SAF path as the mainline behavior.
...
- [ ] Run API 36 Safer Intents strict-matching smoke for OS user shortcut cards, OEM settings helpers, external browser/download/share intents, and GitHub share import; add action, filter, scheme, host, or package constraints where strict resolution drops legitimate launches.
...
- [ ] Evaluate HTTPS hardening readiness for OkHttp traffic, including Android 17 CT default behavior, Certificate Transparency opt-in gaps, and ECH compatibility, after checking GitHub, GitHub download redirects, GameKee, BA media CDN, and MCP loopback behavior.
