...
internal fun BaPageContent(
    ...
    val listScrolling by remember(listState) {
        derivedStateOf { listState.isScrollInProgress }
    }
    val nextCalendarPoolNowMs = remember(state.uiNowMs, sectionTickMs) {
        (state.uiNowMs / sectionTickMs).coerceAtLeast(0L) * sectionTickMs
    }
    var settledCalendarPoolNowMs by remember {
        mutableLongStateOf(nextCalendarPoolNowMs)
    }
    LaunchedEffect(nextCalendarPoolNowMs, listScrolling) {
        if (!listScrolling) {
            settledCalendarPoolNowMs = nextCalendarPoolNowMs
        }
    }
    val calendarPoolNowMs = if (listScrolling) settledCalendarPoolNowMs else nextCalendarPoolNowMs
    val calendarPoolEffectsEnabled = !listScrolling
